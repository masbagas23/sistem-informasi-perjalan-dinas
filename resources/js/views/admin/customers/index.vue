        ...
        <b-card class="text-center">
            ...
            >
                ...
                <template v-slot:cell(category)="row">
                    {{ row.value == 1 ? 'Motor' : 'Mobil' }}
                </template>

                <template v-slot:cell(action)="row">
                    <b-badge
                        v-show="row.item.gmaps_url"
                        title="Petunjuk"
                        class="btn"
                        @click="openInNewTab(row.item.gmaps_url);"
                        pill
                        variant="success"
...
export default {
    ...
    data() {
        return {
            fields: [
                ...
                { key: "name", label: "Nama", thStyle: "text-align: center; width: 250px;", sortable: false },
                { key: "telephone", label: "Telepon", thStyle: "text-align: center; width: 150px;",sortable: false },
                { key: "address", label: "Alamat", sortable: false },
                {
                    key: "action",
                    label: "Aksi",
                    thStyle: "text-align: center; width: 130px;",
                    sortable: false
                }
            ],
    ...
    computed: {
        ...
        tableParams: {
            get() {
                ...
                return this.$store.state.admin.mstCustomer.tableParams;
            },
            set(params) {
                this.$store.commit("mstCustomer/SET_TABLE_PARAMS", params);
            }
        }
    },
    mounted(){
    ...
    methods: {
        ...
        create() {
            this.$bvModal.show("modal-form-customer");
        },
        view(id) {
            this.modelId = id;
            this.create();
        ...
        hideModal(loading) {
            ...
            this.$bvModal.hide("modal-form-customer");
        },
