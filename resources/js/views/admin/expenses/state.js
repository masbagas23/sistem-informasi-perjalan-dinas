...
const mutations = {
    ...
    ASSIGN_FORM(state, payload) {
        state.form = {
            ...
            remaining_cost: payload.remaining_cost,
            is_reimburse: payload.is_reimburse,
            status: payload.status,
            status_reimburse: payload.status_reimburse,
...
const actions = {
    ...
    loadList({ commit }, payload) {
        return new Promise((resolve, reject) => {
            $axios.get(`/expense-list`).then(response => {
                //SIMPAN DATA KE STATE MELALUI MUTATIONS
    ...
    show({ commit }, payload) {
        ...
        return new Promise((resolve, reject) => {
            ...
            $axios.get(`/expenses/${payload}`).then(response => {
    ...
    update({ state, commit }, payload) {
        return new Promise((resolve, reject) => {
            ...
            //MELALUI STATE FORM
            $axios
                .put(`/expenses/${payload}`, state.form)
                .then(response => {
                    //FORM DIBERSIHKAN
                    // commit("CLEAR_FORM");
                    resolve(response.data);
    ...
    validate({ state, commit }, payload) {
        return new Promise((resolve, reject) => {
            ...
            //MELALUI STATE FORM
            $axios
                .post(`/expense-validation/${payload}`, state.form)
                .then(response => {
                    //FORM DIBERSIHKAN
                    // commit("CLEAR_FORM");
                    resolve(response.data);
    ...
    reimburse({ state, commit }, payload) {
        return new Promise((resolve, reject) => {
            //MELAKUKAN REQUEST DENGAN MENGIRIMKAN ID DI URL
            //DAN MENGIRIMKAN DATA TERBARU YANG TELAH DIEDIT
            //MELALUI STATE FORM
            $axios
                .post(`/expense-reimburse/${payload}`, state.form)
                .then(response => {
                    //FORM DIBERSIHKAN
                    // commit("CLEAR_FORM");
                    resolve(response.data);
